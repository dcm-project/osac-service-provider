...
- [OSAC SP Enhancement](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md) — [API Endpoints](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md#api-endpoints), [Idempotent Creation](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md#idempotent-creation), [Ownership Tracking](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md#ownership-tracking), [Status Mapping](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md#status-mapping-osac-to-dcm) — **PR [#96](https://github.com/dcm-project/enhancements/pull/96)** (open, unmerged) is the current source of truth for the Phase 1 (`control-plane`) contract; this spec was drafted directly against that PR's content plus independent verification of every cited upstream source (not the PR's prose alone — see DD-080/090/100 and SC-M3-001/002 below for corrections/extensions found in that process)
...
- OSAC public protos, already vendored in Milestone 2: [`clusters_service.proto`/`cluster_type.proto`](https://github.com/osac-project/fulfillment-service/tree/73ae26e8cb0a476d4b035b18776603f60a361ed9/proto/public/osac/public/v1) — no new vendoring needed
...
- [Design Decisions](../decisions/osac-sp.decisions.md) — DD-080/090/100 (new, this milestone)
...
- **`internal/cluster`** — business logic. Constructs
  `publicv1.NewClustersClient(bootstrap.Conn())` (per M2's DD-020 pattern —
  no new accessor added to `Bootstrap`) and exposes `Create`/`Get`/`List`/
  `Delete` methods operating on the SP's own `Cluster` type, encapsulating
  DCM<->OSAC field translation, the idempotent-create retry, and the shared
  status mapper (§4.5).
...
|         v                                                   |
|              internal/cluster                              |
|   Create: translate spec -> ClusterSpec, set ownership      |
|           labels, Clusters/Create, AlreadyExists->Get (DD-100)|
...
|---|-------|--------|------------|
| 1 | Cluster Create | CREATE | Topic 5 (Status Mapping), Topic 6 (Error Mapping), Milestone 2 `Bootstrap.Conn()` |
...
- **Validates:** REQ-CREATE-010, REQ-CREATE-020, REQ-CREATE-025
- **Given** a request `POST /api/v1alpha1/clusters?id=X` with body `{"spec":{"version":"1.29","nodes":{"worker":{"count":3}},"metadata":{"name":"foo"},"provider_hints":{"osac":{"template_id":"default-hcp"}}}}`
- **When** the handler processes it against a fake `bufconn`-backed `ClustersServer`
- **Then** the fake's recorded `Clusters/Create` call has `Cluster.id` exactly `"X"`, `spec.template` exactly `"default-hcp"`, `spec.node_sets["default-hcp"].size` exactly `3` (or the template's documented node-set key), `spec.metadata.name` exactly `"foo"`, and `spec.release_image` equal to the placeholder table's mapped value for `"1.29"` (not empty, not the literal string `"1.29"`)
...
- **When** processed
- **Then** the fake's recorded `Cluster.metadata.labels` equals exactly `{"team":"platform","dcm.io/managed-by":"dcm","dcm.io/instance-id":"X","dcm.io/service-type":"cluster"}` — not a superset check, not missing the caller's own label
...
- **Then** the fake's second `Clusters/Create` call returns `AlreadyExists`, the SP calls `Clusters/Get("X")`, the HTTP response is `201 Created` with `id` exactly `"X"` and `status` exactly reflecting the **existing** cluster's current state (`"PROGRESSING"` from the Get — not a freshly-fabricated value), and the fake OSAC server's cluster count remains exactly `1` (no duplicate created)
...
- **When** processed
- **Then** the fake's recorded `Clusters/Create` call's `node_sets[key].host_type` field is the empty string (the SP never sets it — OSAC's template fills it server-side), proving `cpu`/`memory`/`storage` were not translated into any OSAC field

#### Dependencies

Depends on Topic 5 (Status Mapping, for the response `status` field),
Topic 6 (Error Mapping), and Milestone 2's `Bootstrap.Conn()`.
...
- **Then** the fake recorded `filter` exactly equal to `this.metadata.labels["dcm.io/managed-by"] == "dcm"` and `limit` exactly `50`, and the response's `results` array has exactly 2 entries whose `id`/`status` fields equal the fake's canned values exactly (not merely `len(results) == 2`)
...
- **Then** the fake's second recorded `List` call has `offset` exactly `50` — proving the opaque token actually carries that value, not just that a non-empty token was returned
...
- **Then** the response is `502 Bad Gateway` (per §4.6's mapping table) — **not** `204`, proving the `NotFound`-tolerance carve-out is not applied to unrelated error codes
...
This is the full canonical set — **not** the 5-value subset
(`PROGRESSING|ACTIVE|DEGRADED|FAILED|DELETED`) that appears in the
enhancement doc's own Status Mapping table (that table only lists the
signals OSAC *currently* drives; `UNAVAILABLE` and `DELETING` are legitimate,
required enum values even though only `UNAVAILABLE` has a real driving
signal today — see DD-090).
...
- **Then** it returns exactly `FAILED`, proving state-level failure is checked before the condition, not merely that both independently-tested cases pass in isolation
...
Every gRPC error surfaced by any `Clusters` RPC (except the two carve-outs
below) MUST be mapped to an HTTP status and an RFC 9457 (`application/problem+json`,
DD-070) body, reusing `internal/httperror.WriteResponse` — no new
error-writing mechanism. Milestone 1's `v1alpha1.ErrorType` enum already
...
- **Then** all three produce an identical HTTP status (`403`) and identical `type` value — proving one shared implementation, not three independently-drifting ones
...
status vocabulary), and **DD-100** (SP-side idempotent-create is a hard
...
Verified directly against OSAC's vendored (Milestone 2) commit
[`73ae26e`](https://github.com/osac-project/fulfillment-service/blob/73ae26e8cb0a476d4b035b18776603f60a361ed9/proto/public/osac/public/v1/cluster_type.proto#L237-L253):
`ClusterState` defines `CLUSTER_STATE_DELETING = 4` and
`CLUSTER_STATE_DELETE_FAILED = 5`. However, `osac-operator`'s
...
has literal `// TODO` stubs for both `handleDelete` and the
`ClusterOrderPhaseDeleting` phase — nothing in OSAC's reconciler path
actually sets either value today, and `private_clusters_server.go`'s
`Delete()` performs no state transition of its own before the generic DAO
delete. `Clusters/Get`/`List` continue returning the object (not `404`)
until OSAC's DAO archives the record once no finalizers remain — deletion
is asynchronous at the API level, and the cluster can disappear from OSAC's
API before the underlying Hosted Control Plane teardown actually finishes.
REQ-STATUS-020's `DELETING`/`DELETE_FAILED` mapping rules are therefore
forward-compatible dead code today, not a currently-exercisable path — do
not expect a `TC-I-*` to observe them without a fake OSAC server that
fabricates a state no real OSAC deployment produces yet. This same finding
was pushed as a corrective commit to enhancement
[PR #96](https://github.com/dcm-project/enhancements/pull/96) before this
spec was drafted, so this spec and the enhancement doc agree.
...
Read [`service-type-definitions.md`](https://github.com/dcm-project/enhancements/blob/main/enhancements/service-type-definitions/service-type-definitions.md#L103-L114,386-427)
directly rather than trusting PR #96's field-mapping table alone. Confirms:
`id`/`status`/`path`/`status_message`/`create_time`/`update_time` are
`ReadOnly: Yes` (system-assigned) on the generic schema — consistent with,
not contradicting, `control-plane` generating/forwarding `id` as a query
parameter rather than a request-body field (DD-080). Confirms the Cluster
schema's field names (`version`, `nodes.control_plane.*`,
`nodes.worker.{count,cpu,memory,storage}`) and the common `metadata.name`
(required)/`metadata.labels` (optional map)/`provider_hints.<provider>`
(free-form `map[string]any`) fields. No conflicts found.

**Deliberate deviation from the enhancement doc's example response JSON:**
that example shows `nodes: {control_plane: {ready, total}, worker: {ready,
total}}`, but OSAC's actual `ClusterStatus`/`ClusterNodeSet` proto types
(verified directly, same commit) have no `ready` count field and no
control-plane node-set bucket at all (Hosted Control Planes hide it
entirely). Rather than inventing an unverifiable `ready`/`total`
computation, this milestone's `Cluster` response schema echoes OSAC's
`status.node_sets` map directly (`{<key>: {host_type, size}}`) — a shape
this spec can actually verify against a fake server, instead of a
plausible-looking shape borrowed from an illustrative example that does not
correspond to any real OSAC field.

**`version` is Create-response-only, not reconstructed on Get/List:** the
enhancement's own [Version Translation](https://github.com/dcm-project/enhancements/blob/main/enhancements/osac-sp/osac-sp.md#version-translation)
section describes only a one-directional (DCM version → OSAC
`release_image`) mapping, explicitly deferring the full matrix to Milestone
6; no reverse mapping is documented or required anywhere in scope. This
milestone's Create response echoes the request's own `spec.version` input
(no OSAC round-trip needed); Get/List responses omit `version` rather than
attempting an unsupported reverse translation from `release_image`. This
keeps the SP genuinely stateless for M3 — no local ID-mapping/version-cache
store is introduced, consistent with DCM's identifier and OSAC's `Cluster.id`
always being the same value (DD-080's ID Mapping note), which is the only
reason no such store is needed at all.
...
| Prefix | Topic | Count |
|--------|-------|-------|
| REQ-CREATE-NNN | 4.1: Cluster Create | 7 |
