...
survive that decision. TLS (DD-233) is deliberately the only wire-level
...
## DD-233: TLS to the fulfillment service is unconditional — `TLSEnabled` removed, no insecure fallback
...
not connection-level) could travel in plaintext by default. Given DD-232
