...
| TC-U-002 | Applies documented defaults when optional vars are unset | REQ-XC-CFG-010 | Unset all optional vars; assert `server.address == ":8080"`, `server.shutdownTimeout == 15s`, `server.requestTimeout == 30s`, `osac.probeTimeout == 5s`, `provider.clusterName == "osac-sp-cluster"`, `provider.vmName == "osac-sp-vm"`. |
...
| TC-U-012 | Builds TLS transport credentials with the system default CA pool when no custom CA is configured | REQ-OSAC-040, AC-OSAC-045 | `tlsCertFile` unset (DD-229: TLS is unconditional, there is no "disabled" case anymore); call the internal dial-options builder; assert the returned credentials' `Info().SecurityProtocol == "tls"`. |
| TC-U-013 | Builds TLS transport credentials with a configured custom CA | REQ-OSAC-040, AC-OSAC-040 | `tlsCertFile` points to a test CA PEM fixture; assert the returned credentials' `Info().SecurityProtocol == "tls"` and the loaded cert pool contains exactly the test CA's subject. |
...
| TC-U-153 | `waitForReadyUntilCancelled` retries after a readiness window times out, instead of giving up permanently | REQ-REG-052, AC-REG-031 | Regression test for a real bug the kind-based e2e infra caught under genuine CPU contention (DD-141). Point `WithReadinessTiming` at a released loopback address (nothing listening) so the first window times out; after at least one window has elapsed, start listening on that same address; assert `waitForReadyUntilCancelled` still returns `nil` once the server becomes reachable, rather than having already given up after the first window. |
| TC-U-154 | `waitForReadyUntilCancelled` stops retrying only on context cancellation, not on an elapsed window | REQ-REG-052, AC-REG-031 | With nothing ever listening at the probed address, let at least one window elapse (confirming a retry has begun), then cancel the context; assert the call returns promptly with a non-nil error — cancellation, not the elapsed window, is what ends the retry loop. |
...
| TC-U-096 | `run` wraps and returns an OSAC bootstrap construction failure | REQ-OSAC-040 (negative case) | Set `SP_OSAC_TLS_CERT_FILE` to a nonexistent path; call `run`; assert it returns a non-nil error mentioning "creating OSAC client bootstrap". |
