...
| TC-U-118 | `Clusters.Create` sets `CLUSTER_STATE_READY`, a kubeconfig Secret reference, and round-trips via `Get`/`List` | REQ-MOCK-020, REQ-MOCK-030, REQ-MOCK-040, REQ-MOCK-050, REQ-MOCK-120, AC-MOCK-030 | `Create` with `id="x"`, `status` unset; assert `status.state == CLUSTER_STATE_READY` and `status.kubeconfig_secret.id == "mock-kubeconfig-x"`; call `Get("x")` and assert the returned object is `proto.Equal` to `Create`'s; call `List()` and assert it contains exactly that object. |
...
| TC-U-155 | A created Cluster's kubeconfig reference resolves through `Secrets.Get`; unknown Secret ID is `NotFound` | REQ-MOCK-120, AC-MOCK-130 | Create a cluster with `id="x"`; assert its status reference is `mock-kubeconfig-x`; call private `Secrets.Get` with that exact ID and assert type `SECRET_TYPE_KUBECONFIG` plus exact non-empty raw bytes at `data["kubeconfig"]`; call with an unknown Secret ID and assert `NOT_FOUND`. |
...
| TC ID | Test Name | Validates | Description |
|-------|-----------|-----------|-------------|
| TC-I-031 | A real `osac.Bootstrap` authenticates, probes, and retrieves a kubeconfig Secret from the mock binary over real TLS | REQ-MOCK-010, REQ-MOCK-070, REQ-MOCK-080, REQ-MOCK-090, REQ-MOCK-110, REQ-MOCK-120, REQ-MOCK-140, AC-MOCK-120, AC-MOCK-130 | Start the real `run()` (env-var config, both real listeners, public and private gRPC services over real TLS via `ServerTLSConfig`, the OIDC stub); construct production `osac.Bootstrap`, wait for a valid token and successful probe; then create cluster `id="x"` through its real public client, assert the exact kubeconfig Secret reference, call private `Secrets.Get` through the same connection, and assert the exact non-empty raw bytes. |
...
|---|---|---|---|---|---|
| `Clusters`/`ComputeInstances` CRUD | REQ-MOCK-020, 030, 040, 050, 060, 120 | AC-MOCK-010, 020, 030, 040, 050, 060 | 12 (TC-U-114..125) | — | TC-U-118 asserts the Cluster's deterministic kubeconfig Secret reference; TC-U-155 covers resolving it. Other CRUD semantics remain in-memory and need no distinct wire case. |
...
| Binary wiring (`test/cmd/osac-mock-provider`) | REQ-MOCK-010, 070, 080, 090, 110, 120, 140 | AC-MOCK-120, 130 | 9 (TC-U-144..151, 157) | 1 (TC-I-031) | `run`/`serveUntilDone` are 100% unit-covered on their own (TC-U-144..151); TC-I-031 proves the public and private services are reachable over real TLS with a real `osac.Bootstrap`. `main`/`mainRun`'s happy path is a documented coverage exception (real-OS-signal-dependent), mirroring `cmd/osac-service-provider/main.go`'s own accepted gap. |
| Cluster kubeconfig Secret / `ClusterTemplates` | REQ-MOCK-120, 130 | AC-MOCK-130, 140 | 2 (TC-U-155..156) | TC-I-031 (Secrets wire path) | TC-U-155 and TC-I-031 cover the v0.0.107 Secret-reference contract; TC-U-156 remains the regression test for `ClusterTemplates/Get`. |
| **Total** | 15 | 15 | **42** | **1** | |
