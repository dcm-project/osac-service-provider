...
| TC-TB-200 | osac-sp-initiated cluster Create routes through fulfillment-service Hub dispatch to a real ClusterOrder | REQ-TB-100, AC-TB-060 | The workflow builds the `osac` CLI from the `fulfillment-service/v0.0.107` monorepo tag, registers a Hub with `osac create hub --name`, then validates the full flow: osac-sp `POST /api/v1alpha1/clusters?id=...` → fulfillment-service dispatch → real ClusterOrder CR on Hub → osac-operator/osac-aap-mock drive to Ready. The request uses `1.29` as a representative live version; matrix-wide support and newest-z-stream selection are covered by M6's unit/integration cases. |
