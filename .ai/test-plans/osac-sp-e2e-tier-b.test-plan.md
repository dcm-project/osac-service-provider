...
| TC-TB-080 | The e2e suite creates a `ClusterOrder` CR directly against the kind cluster's own API server | REQ-TB-070, REQ-TB-100 | Deliberately exercises the direct-reconciliation path rather than `osac-sp`/`fulfillment-service`'s dispatch chain (covered separately by TC-TB-200). Asserts the CR is accepted (the fixture-grade CRD, DD-213-adjacent precedent, has no schema to reject it) and osac-operator's `ClusterOrderReconciler` picks it up (a non-empty `.status.phase` appears) before the terminal-state wait begins. |
...
| TC-TB-200 | osac-sp-initiated cluster Create routes through fulfillment-service Hub dispatch to a real ClusterOrder | REQ-TB-100, AC-TB-060 | The workflow builds the `osac` CLI from the `fulfillment-service/v0.0.107` monorepo tag, registers a Hub with `osac create hub --name`, and registers `1.29` as the sole live ClusterVersion. It then validates the full flow: osac-sp `POST /api/v1alpha1/clusters?id=...` → fulfillment-service dispatch → real ClusterOrder CR on Hub → osac-operator/osac-aap-mock drive to Ready. Matrix-wide support and newest-z-stream selection are covered by M6's unit/integration cases. |
| TC-TB-210 | Cluster Get returns the kubeconfig from a real Fulfillment Service Secret | REQ-TB-030, REQ-TB-130, AC-TB-070, REQ-GET-020 | Against real Fulfillment Service v0.0.107, create a dedicated non-shared tenant with a unique DNS domain and wait for IDP sync; create a Cluster in that tenant through the pinned `osac` CLI, create a kubeconfig Secret through the real private API, and update the Cluster's status to `CLUSTER_STATE_READY` with that Secret ID. Then call osac-sp's real `GET /api/v1alpha1/clusters/{id}` and assert `200`, status exactly `ACTIVE`, kubeconfig exactly equal to the standard-base64 encoding of the test bytes, and decoding returns those exact bytes. A non-shared tenant avoids requiring the Tier B stack to configure Vault; no real OpenShift cluster provisioning is needed. |
...
| Phase 2 infra/terminal-state (`ClusterOrder` + `BareMetalInstance`, direct and dispatched CR create) | REQ-TB-070, REQ-TB-080, REQ-TB-100, REQ-TB-110 | AC-TB-030, AC-TB-040, AC-TB-060 | 6 (TC-TB-060/080/090/110/120/200) | Real reconciliation through a real AAP-layer fake (`ClusterOrder`) and real BMFO against a static host fixture (`BareMetalInstance`, DD-226/227). TC-TB-200 additionally covers the osac-sp → fulfillment-service Hub-dispatch path using the fixed `v0.0.107` CLI; the direct CR path remains covered by TC-TB-080/090. Supersedes the retired `TC-TB-100` deploy-only placeholder (DD-216). |
| Secret-backed Cluster Get | REQ-TB-130 | AC-TB-070 | 1 (TC-TB-210) | Exercises the real osac-sp HTTP handler, real Fulfillment Service private `Clusters/Update` and `Secrets/Get`, and the exact base64 response contract without requiring a provisioned OpenShift cluster. |
...
| **Total** | 12 | 7 | **30** | Phase 1 has 4 planned runtime/static checks and two integration-tier-sufficient failure branches; Phase 2 covers reconciliation, Hub dispatch, and real Secret-backed Cluster Get against pinned Fulfillment Service v0.0.107. |
