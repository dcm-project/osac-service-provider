...
| TC ID | Test Name | Validates | Description |
|-------|-----------|-----------|-------------|
| TC-U-560 | `GetTemplate` (job template lookup by name) returns a real `{count, results}` body for any requested name | REQ-TB-080 | `GET /v2/job_templates/?name={name}` returns `200` with `count: 1`, `results: [{id, name}]` — the mock accepts any name (from-scratch fake, DD-212), no fixture template list to keep in sync. |
...
| TC-U-564 | `GetJob` reports a launched job as `status: "successful"` immediately, with `started`/`finished` both populated | REQ-TB-080, REQ-TB-100, DD-213 | `GET /v2/jobs/{id}/` on a job launched via TC-U-562 returns `200` with `status: "successful"` on the very first call — no pending/running transition (DD-213) — matching the exact string `osac-operator/pkg/provisioning/aap_provider.go`'s `mapAAPStatusToJobState` maps to `JobStateSucceeded`. |
...
| TC-U-568 | `CancelJob` on an already-terminal job returns `405`, not a silent success | REQ-TB-080, DD-213 | Calling cancel twice: second call returns `405` (mirrors `MethodNotAllowedError`) — deliberately fail-safe rather than permissive, since a silently-succeeding cancel would make the mock more forgiving than real AAP and lose fidelity value for `AAPProvider.isReadyForDeprovision`'s 405-detection branch. |
| TC-U-569 | Every endpoint requires an `Authorization: Bearer` header to be present, but never validates its content | REQ-TB-080, NFR-TB-030 | A request with no `Authorization` header at all is still accepted (`200`) — same deliberate permissiveness precedent as `DD-132`'s OIDC stub, since NFR-TB-030 scopes fidelity to "no real Ansible/hardware access," not AAP-layer auth, and `REQ-TB-080`/`AC-TB-030` are scoped to reconciliation reaching a terminal state. Documents the choice as a real assertion (a passing "permissive by design" test), not an accidental gap. |
...
| TC ID | Test Name | Validates | Description |
|-------|-----------|-----------|-------------|
| TC-TB-060 | `osac-operator`, BMFO, and `osac-aap-mock` all reach `Ready` within the bounded wait, on top of Phase 1's already-`Ready` stack | REQ-TB-070, AC-TB-030 (given clause) | Same readiness-polling discipline as TC-TB-010, extended to the 3 new Phase 2 pods plus the 4 vendored CRDs' existence (`kubectl get crd`) — assert this completes within NFR-TB-010's overall 25-minute budget. |
| TC-TB-080 | The e2e suite creates a `ClusterOrder` CR directly against the kind cluster's own API server | REQ-TB-070, REQ-TB-100 | Not yet routed through `osac-sp`/`fulfillment-service`'s dispatch chain — see DD-217, tracked in [#47](https://github.com/dcm-project/osac-service-provider/issues/47). Asserts the CR is accepted (the fixture-grade CRD, DD-212-adjacent precedent, has no schema to reject it) and osac-operator's `ClusterOrderReconciler` picks it up (a non-empty `.status.phase` appears) before the terminal-state wait begins. |
...
| `osac-aap-mock` unit coverage | REQ-TB-080 | — | 14 (TC-U-560..573) | Counts toward the repo's 100%-unit-coverage gate, unlike the `TC-TB-*` rows below. |
| Phase 2 infra/terminal-state (`ClusterOrder`, direct CR create) | REQ-TB-070, REQ-TB-080, REQ-TB-100 | AC-TB-030 | 3 (TC-TB-060/080/090) | The Phase 2 deliverable — real reconciliation through a real AAP-layer fake, scoped to `ClusterOrder` only (DD-215) and a direct CR create rather than an `osac-sp`-driven one (DD-217, #47). |
